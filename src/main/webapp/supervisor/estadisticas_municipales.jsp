...
<body class="d-flex flex-column min-vh-100">
	...
	<div class="wrapper">
		...
		<div class="container-fluid m-3" style="min-height: 75vh">
			<h2 class="mb-4">Estadísticas municipales: </h2>
			...
			<c:if test="${accesoGeneral == true}">
				<div>
					Elija una localidad:</label> 	
					<select name="localidad" id="localidad" class="selectpicker myselect" data-live-search="true" >
			...
			<c:if test="${tieneAcceso == true}">
				<div>
					<label for="localidad" class="form-label">Localidad: <c:out value="${localidad}"></c:out></label> 	
				</div>
			...
			<input type="hidden" id="ambito" value="MUNICIPAL">
			<input type="hidden" id="lugar" value="${localidad}">
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.permitBtn').on('click', function() {
			let acceso = prompt("Por favor, indica a qué localidad quieres acceder", "");
			if(acceso != "") {
				window.location = "pedir_permisos?id=MUNICIPAL&acceso="+acceso;
			}
		});
	});
</script>
